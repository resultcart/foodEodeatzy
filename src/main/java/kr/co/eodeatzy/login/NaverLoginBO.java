...
public class NaverLoginBO {
	...
	private final static String CLIENT_ID = "zLjKmY9zcfk03B1zLBFT";
    private final static String CLIENT_SECRET = "l_kSV2VYtq";
    private final static String REDIRECT_URI = "http://localhost:8080/eodeatzy/login/callbackNaver";
	...
	public String getAuthorization(HttpSession session) {
		...
		OAuth20Service auth20Service = new ServiceBuilder().apiKey(CLIENT_ID).apiSecret(CLIENT_SECRET).callback(REDIRECT_URI).state(state).build(NaverAuth.instance());
	...
	public String getUserProfile(OAuth2AccessToken accessToken) throws IOException{
		...
		OAuth20Service auth20Service = new ServiceBuilder().apiKey(CLIENT_ID).apiSecret(CLIENT_SECRET).callback(REDIRECT_URI).build(NaverAuth.instance());
