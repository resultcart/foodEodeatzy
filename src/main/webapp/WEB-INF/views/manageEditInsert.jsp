...
$(document).ready(function(){
    ...
    if (document.getElementById("hidden_upId").value == 1) {					//게시판_테이블
    	fn_hidden();
    	$("#sht_board").show();
    } else if (document.getElementById("hidden_upId").value == 2) {				//장바구니_테이블
    	fn_hidden();
    	$("#sht_cart").show();
    } else if (document.getElementById("hidden_upId").value == 3) {				//카테고리분류_테이블
    	fn_hidden();
    	$("#sht_category").show();			
    } else if (document.getElementById("hidden_upId").value == 4) {				//콤보리스트_테이블
    	fn_hidden();
    	$("#sht_combo").show();				
    } else if (document.getElementById("hidden_upId").value == 5) {				//댓글_테이블
    	fn_hidden();
    	$("#sht_comment").show();		
    } else if (document.getElementById("hidden_upId").value == 6) {				//메뉴_테이블
    	fn_hidden();
    	$("#sht_menu").show();				
    } else if (document.getElementById("hidden_upId").value == 7) {				//주문내역_테이블
    	fn_hidden();
    	$("#sht_order").show();				
    } else if (document.getElementById("hidden_upId").value == 8) {				//주문내역_상세_테이블
    	fn_hidden();
    	$("#sht_order_detail").show();		
    } else if (document.getElementById("hidden_upId").value == 9) {				//사업장정보_테이블
    	fn_hidden();
    	$("#sht_store").show();
    } else if (document.getElementById("hidden_upId").value == 10) {			//사업장_주소_테이블
    	fn_hidden();
    	$("#sht_store_address").show();
    } else if (document.getElementById("hidden_upId").value == 11) {			//사업자정보_테이블
    	fn_hidden();
    	$("#sht_user_b").show();
    } else if (document.getElementById("hidden_upId").value == 12) {			//개인사용자정보_테이블
    	fn_hidden();
    	$("#sht_user_p").show();
    } else if (document.getElementById("hidden_upId").value == 13) {			//개인사용자_주소_테이블
    	fn_hidden();
    	$("#sht_user_p_address").show();
    } else if (document.getElementById("hidden_upId").value == 14) {			//찜목록_테이블
    	fn_hidden();
    	$("#sht_zzim").show();
    } else {
      //$("#sht_user_p").hide();
	...
	<div class="row justify-content-center">
		<div class="col-lg-16">
			...
			<div id="sht_board" name="sht_board">
				<form name="fm" method="POST" action="${contextPath}/manage/manageInsertTbBoard">
				    <div class="box" id="sht_board_dtl">
					<div class="box-header">
						<h4 class="box-title">insert 페이지</h4>
						<h4 class="box-title">상세조회</h4>
						<button class="main-btn" id="btnExcute1" name="btnExcute1" >처리</button>
					</div>
					<div class="box-body">
						<div class="form-group">
							<label>게시글ID</label><input type="text" id="b_id" name="b_id"  class="form-control"  />
							<label>작성자</label><input type="text" name="b_writer" id="b_writer" class="form-control"   />
							<label>게시판타입</label><input type="text" name="b_type" id="b_type" class="form-control"     />
							<label>제목</label><input type="text" name="b_title" id="b_title" class="form-control"   />
							<label>내용</label><input type="text" name="b_content" id="b_content" class="form-control"   />
							<label>사진파일</label><input type="text" name="b_img_upload" id="b_img_upload" class="form-control"   />
							<label>제보게시글상태</label><input type="text" name="b_state" id="b_state" class="form-control"   />
							<label>등록일시</label><input type="text" name="b_datetime" id="b_datetime" class="form-control"   />
						</div>
					</div>
					</div>
				</form>
			...
			<form name="fm" method="POST" action="${contextPath}/manage/manageInsertTbCombo">
			    <div class="box">
				<div class="box-header">
					<h4 class="box-title">상세조회</h4>
					<button class="main-btn" id="btnExcute4" name="btnExcute4" >처리</button>
				</div>
				<div class="box-body">
					<div class="form-group">
