...
$(document).ready(function(){
    ...
    $("#btnSelect").click(function(){
		location.href="${contextPath}/board/noticeSelectList";
	});
	//공지사항 등록
	...
	$("#btnUpdate").click(function(){
		var id = 0;
		id = document.getElementById("ip_b_id").value;
		if (id == 0) {
			alert("수정하려는 항목을 선택해 주세요.");	
		} else {
			location.href="${contextPath}/board/noticeUpDel?b_id="+id+"&btnGb="+2;
		}
	});
	...
	$("#btnDelete").click(function(){
		...
		if (id == 0) {
			...
			location.href="${contextPath}/board/noticeUpDel?b_id="+id+"&btnGb="+3;
		}
	});
                  ...
                  <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                  	<c:forEach items="${noticeList}" var="list">
                    	<tr class="text-gray-700 dark:text-gray-400">
                    		<td class="px-3 py-2">${list.b_id}</td>
							<td class=" px-3 py-2">${list.b_writer}</td>
							<td class=" px-3 py-2">${list.b_title}</td>
							<td class=" px-3 py-2">${list.b_content}</td>
